...
run-spark-master:
	...
	docker run -d -p 8080:8080 --name spark-master -h spark-master spark-master

run-spark-worker-cluster:
	echo "Runs 4 Spark workers 3GB RAM(12GB total), 1 core each"
	for number in 1 2 3 4 ; do \
		docker run -d --cpuset-cpus="$$number" --memory="3072M" --memory-swap="3072M" --name spark-worker-$$number --link spark-master:spark-master spark-worker ;\
	done
